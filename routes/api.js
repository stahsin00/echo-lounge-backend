...
router.post('/serve', async (req, res) => {
    ...
    try {
        ...
        customer.history.push({ role: 'system', content: `Remember to respond in strict JSON format following the structure: { "message": "[The response message from you as the customer]", "expression": "[The facial expression to display (must be one of the following: "smily", "angry", "dizzy", "flushed", "sad", "sheepish", "inLove", "laughing", "wink", "crying", "surprised", "excited", "neutral"]", "tone": "[The tone of the message (e.g., "happy", "sad", "neutral", etc. The tone must a simple single word)], "memory": [OPTIONAL, include any new fact you mentioned in your message that would be good to remember long term]"} Don't forget to specify any new memories. Do not include anything except the JSON`})

        const data = await getChatGptResponse(customer.history);
        const response = parseCustomerResponse(data);
        const { message, expression, tone, memory } = response;
